...
最后将自定义 base64 解码结果逆 LFSR 还原，再按 \`CryptoEngine.transform()\` 的逆过程逐字节恢复明文，即可得到花括号内部内容为 \`abyssal\_vows\_bind\_the\_last\_protocol\`，最终拼接出完整 flag。
...
target\_b64 = 'T8GHtJzYDRX7siXzWKLJCGlrpi1sVwnvMhZCPj3jwDWYU4Y='

char\_to\_idx = {c: i for i, c in enumerate(alphabet)}

decoded = \[\]

bits = bit\_count = 0

for c in target\_b64:

if c == '=':

break

bits = (bits &lt;&lt; 6) | char\_to\_idx\[c\]
...
decoded.append((bits &gt;&gt; bit\_count) & 0xff)
...
new\_state = (state &gt;&gt; 1) | (feedback &lt;&lt; 7)

xor\_stream.append((counter + new\_state) & 0xff)

state, counter = new\_state, counter + 7

expected = bytes(d \^ x for d, x in zip(decoded, xor\_stream))
...
before\_xor = x \^ mask\[i % len(mask)\]

shift = shifts\[i % len(shifts)\]

char\_code = ((before\_xor - 32 - shift) % 95) + 32

inner.append(chr(char\_code))
...
\`ISCC{abyssal\_vows\_bind\_the\_last\_protocol}\`
